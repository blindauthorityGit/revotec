            ...
            <div className="w-full mb-6 font-body">
                <div className="relative w-full h-96 rounded-lg overflow-hidden">
                    <div
                        ...
                        style={{ backgroundImage: `url(${urlFor(heroImage).url()})` }}
                    />
                    <div className="absolute inset-0 bg-black/40" />
                    <div className="relative p-4 flex flex-col justify-center h-full text-white space-y-2">
                        <h3 className="font-headline uppercase text-xl leading-tight">{contentTitle}</h3>
                        <P className="text-xs opacity-75 py-4 leading-snug">{heroIntro}</P>
                        {slug && (
                            <Link href={`/referenzen/${slug}`}>
                                <span className="inline-block underline font-semibold">Mehr erfahren</span>
                            </Link>
                        )}
                    </div>
                </div>
        ...
        >
            ...
            >
                ...
                <motion.div variants={contentVariants} transition={{ delay: 0.15 }}>
                    <H3 className="text-2xl font-bold">{contentTitle}</H3>
                </motion.div>
                {heroIntro && (
                    <motion.div variants={contentVariants} transition={{ delay: 0.2 }}>
                        <P className="text-sm leading-snug">{heroIntro}</P>
                    </motion.div>
                )}
                {slug && (
                    <motion.div variants={contentVariants} transition={{ delay: 0.25 }}>
                        <Link href={`/referenzen/${slug}`}>
                            <span className="inline-block mt-4 border-b-2 border-white font-semibold">
                                Mehr erfahren
                            </span>
                        </Link>
                    </motion.div>
